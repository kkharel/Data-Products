...
- Let us consider a set of observations $y_1, y_2, ..., y_n$, the likelihood function $L$
...
density function or the probability mass function which is indexed by parameter $\theta$.

- Then we construct a likelihood function using the assumption that the data are iid which
is simply the product of density or pmf evaluated at each sample data points $y_i$.
...
squared that hits the highest part of the surface. 
...
- Suppose we observe ten successes in 25 trials. Here we are not asking
...
fig <- fig %>% layout(title = "Theta vs Likelihood",
